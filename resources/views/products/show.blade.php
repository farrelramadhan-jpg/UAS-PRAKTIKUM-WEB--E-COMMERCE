        ...
        @auth
            <div class="bg-gray-50 rounded-lg p-6 mb-8 border border-gray-200">
                ...
                <form action="{{ route('comments.store') }}" method="POST">
                    ...
                    <div class="inline-flex flex-row-reverse gap-1">
                        @for($i = 5; $i >= 1; $i--)
                            <input type="radio" name="rating" value="{{ $i }}" id="rating-{{ $i }}" class="sr-only peer" {{ $i === 5 ? 'checked' : '' }}>
                            
                            <label for="rating-{{ $i }}" class="peer cursor-pointer text-gray-300 hover:text-yellow-400 peer-hover:text-yellow-400 peer-checked:text-yellow-400">
